...
{
        debug(LOG_INFO, "Clearing the portalservers list");
        iptables_fw_clear_portalservers();
}

/** Add the necessary firewall rules to whitelist the portalservers
 */
void
fw_set_portalservers(void)
{
        debug(LOG_INFO, "Setting the portalservers list");
        iptables_fw_set_portalservers();
}

/** Remove all plat server firewall whitelist rules
 */
void
fw_clear_platservers(void)
{
        debug(LOG_INFO, "Clearing the platservers list");
        iptables_fw_clear_platservers();
}

/** Add the necessary firewall rules to whitelist the platservers
 */
void
fw_set_platservers(void)
{
        debug(LOG_INFO, "Setting the platservers list");
