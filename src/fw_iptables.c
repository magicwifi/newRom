...
iptables_fw_clear_portalservers(void)
{
        ...
        void
iptables_fw_set_portalservers(void)
{
        const s_config *config;
        t_serv *portal_server;

        config = config_get_config();

        for (portal_server = config->portal_servers; portal_server != NULL; portal_server = portal_server->next) {
                if (portal_server->last_ip && strcmp(portal_server->last_ip, "0.0.0.0") != 0) {
                        iptables_do_command("-t filter -A " TABLE_WIFIDOG_PORTALSERVERS " -d %s -j ACCEPT", portal_server->last_ip);
                        iptables_do_command("-t nat -A " TABLE_WIFIDOG_PORTALSERVERS " -d %s -j ACCEPT", portal_server->last_ip);
                }
        }
...
void
iptables_fw_set_platservers(void)
{
        const s_config *config;
        t_serv *plat_server;
        ...
        for (plat_server = config->plat_servers; plat_server != NULL; plat_server = plat_server->next) {
                if (plat_server->last_ip && strcmp(plat_server->last_ip, "0.0.0.0") != 0) {
                        iptables_do_command("-t filter -A " TABLE_WIFIDOG_PLATSERVERS " -d %s -j ACCEPT", plat_server->last_ip);
                        iptables_do_command("-t nat -A " TABLE_WIFIDOG_PLATSERVERS " -d %s -j ACCEPT", plat_server->last_ip);
                }
        }
...
iptables_fw_init(void)
{
	...
	iptables_do_command("-t filter -A " TABLE_WIFIDOG_WIFI_TO_INTERNET " -j " TABLE_WIFIDOG_PORTALSERVERS);
