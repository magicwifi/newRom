...
thread_fetchconf(void *arg)
{
	...
	while (1) {
		...
		fetchconf();
		debug(LOG_DEBUG, "Running fetchconf()");
...
parse_auth_server(char *ptr,s_config	*config)
{
	...
	new->authserv_hostname = safe_strdup(host);
...
fetchconf()
{
	...
	if (sockfd == -1) {
		debug(LOG_DEBUG, "Failed to connect_auth_server ,sockfd == -1");
		return;
	}
